...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s',
                    handlers=[logging.FileHandler("log_final_definitiv.log", mode='w', encoding='utf-8'),
                              logging.StreamHandler()])
writer = SummaryWriter("logs_final_definitiv/" + datetime.now().strftime("%Y%m%d-%H%M%S"))
...
# --- Modelul Ansamblu (cu corecția aplicată) ---
class EmotionEnsemble(nn.Module):
    ...
    def forward(self, x):
        # CORECȚIE: Am eliminat `with torch.no_grad():` pentru a permite antrenarea
...
def run_training(model, train_loader, val_loader, criterion, optimizer, scheduler, scaler, num_epochs, patience, device, model_path):
    ...
    for epoch in range(num_epochs):
        ...
        val_loss, val_f1_macro, _, _, _ = evaluate(model, val_loader, criterion, device)
...
def evaluate(model, data_loader, criterion, device, thresholds=None):
    ...
    # ÎMBUNĂTĂȚIREA TA: Calculăm F1 per clasă
    f1_per_class = f1_score(all_labels_np, all_preds_np, average=None, zero_division=0)
    return avg_loss, f1_macro, all_labels_np, np.vstack(all_outputs), f1_per_class

def find_best_thresholds(model, val_loader, device):
    logging.info("Se caută pragurile optime de decizie pe setul de validare...")
    _, _, all_labels_np, all_outputs_np, _ = evaluate(model, val_loader, nn.BCEWithLogitsLoss(), device)
...
def main():
    ...
    train_labels = (train_dataset.annotations[EMOTIONS] > 0).astype(int)
    pos_counts = train_labels.sum(); neg_counts = len(train_labels) - pos_counts
    ...
    model_path = os.path.join(args.models_dir, "model_licenta_definitiv.pth")
    ...
    # ÎMBUNĂTĂȚIREA TA: Folosim `weights_only=True`
    ...
    test_loss, test_f1, _, _, f1_per_class = evaluate(model, test_loader, criterion, device, thresholds=best_thresholds)
    
    logging.info("--- REZULTAT FINAL DE LICENȚĂ ---")
    logging.info(f"Performanță Generală - Loss Test: {test_loss:.4f}, F1-Macro Test: {test_f1:.4f}")
    logging.info("Performanță Detaliată per Emoție:")
    # ÎMBUNĂTĂȚIREA TA: Afișăm F1 per clasă
    for i, emotion in enumerate(EMOTIONS):
        logging.info(f"F1 Score pentru '{emotion}': {f1_per_class[i]:.4f}")
