# 4_antreneaza_model_performant.py (Versiune Corectată Final)
import torch
...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s',
                    handlers=[logging.FileHandler("log_antrenament_performant.log", mode='w', encoding='utf-8'),
                              logging.StreamHandler()])
writer = SummaryWriter("logs_performant/" + datetime.now().strftime("%Y%m%d-%H%M%S"))
...
# --- Model cu Dropout ---
def get_performant_model(num_classes, dropout_rate=0.5):
...
def main():
    parser = argparse.ArgumentParser(description="Script Performant de Antrenament")
    ...
    model = get_performant_model(len(EMOTIONS)).to(device)
    ...
    model_path = os.path.join(args.models_dir, "model_performant_best.pth")
