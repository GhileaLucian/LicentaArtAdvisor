...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s',
                    handlers=[logging.FileHandler("log_antrenament_campion.log", mode='w', encoding='utf-8'),
                              logging.StreamHandler()])
writer = SummaryWriter("logs_campion/" + datetime.now().strftime("%Y%m%d-%H%M%S"))
...
# --- Transformări cu Augmentare Puternică ---
def get_transforms(img_size=224):
    return {
        'train': transforms.Compose([
            transforms.RandomResizedCrop(img_size, scale=(0.7, 1.0)),
            transforms.TrivialAugmentWide(), # Augmentare automată și puternică
            transforms.ToTensor(),
...
class EnsembleModel(nn.Module):
    def __init__(self, num_classes, dropout_rate=0.5):
        ...
        self.effnet.classifier = nn.Sequential(
            ...
            nn.Linear(in_features_effnet, num_classes)
        )
        ...
        self.vit.heads.head = nn.Linear(in_features_vit, num_classes)
    def forward(self, x):
        return (self.effnet(x) + self.vit(x)) / 2

# --- Funcțiile de Antrenament și Evaluare (cu mici ajustări) ---
...
def evaluate(model, data_loader, criterion, device, thresholds=None):
    ...
    with torch.no_grad():
        for inputs, labels in data_loader:
            ...
            # Folosim praguri optimizate dacă sunt disponibile
            if thresholds is not None:
                preds = torch.sigmoid(outputs) > thresholds.to(device)
            else:
    ...
    return avg_loss, f1_macro, all_labels_np, torch.sigmoid(outputs).cpu()

# --- Funcție Nouă: Optimizarea Pragului de Decizie ---
def find_best_thresholds(model, val_loader, device):
    logging.info("Se caută cele mai bune praguri de decizie pe setul de validare...")
    model.eval()
    all_labels, all_outputs = [], []
    with torch.no_grad():
        for inputs, labels in tqdm(val_loader, desc="Găsire praguri"):
            if inputs.nelement() == 0: continue
            inputs, labels = inputs.to(device), labels.to(device)
            with torch.amp.autocast(device_type="cuda"):
                outputs = model(inputs)
    ...
    for i in range(all_outputs_np.shape[1]):
        ...
        for threshold in np.linspace(0.1, 0.9, 81):
            ...
            f1 = f1_score(all_labels_np[:, i], preds)
            if f1 > best_f1:
                best_f1 = f1
                best_thresholds[i] = threshold
    logging.info(f"Praguri optime găsite: {best_thresholds}")
    return torch.tensor(best_thresholds)

# --- Funcția Principală ---
def main():
    parser = argparse.ArgumentParser(description="Script Campion de Antrenament")
    ...
    parser.add_argument("--batch_size", type=int, default=8) # Reducem pentru ansamblu
    ...
    # --- MODIFICARE: Folosim doar 8 emoții de bază ---
    ...
    # WeightedRandomSampler rămâne esențial
    ...
    model = EnsembleModel(len(EMOTIONS)).to(device)
    ...
    model_path = os.path.join(args.models_dir, "model_campion_best.pth")
    ...
    logging.info("Antrenament finalizat. Se optimizează pragurile de decizie...")
    ...
    logging.info(f"Performanță Finală (cu praguri optimizate) - Loss Test: {test_loss:.4f}, F1-Macro Test: {test_f1:.4f}")
